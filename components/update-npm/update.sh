...
update() {
  ...
  execute_command_on_container "bash -c 'find /root/nginx-proxy-manager-${RELEASE} -type f -name \"*.conf\" -exec sudo sed -i \"s+include conf.d+include /etc/nginx/conf.d+g\" {} +'"
