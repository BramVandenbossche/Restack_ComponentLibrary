...
update() {
  local RELEASE=$(curl -s https://api.github.com/repos/nginx-proxy-manager/nginx-proxy-manager/releases/latest | grep "tag_name" | awk '{print substr($2, 3, length($2)-4) }')
  ...
  messages+=("$(echo_message "Updating Nginx Proxy Manager" false)")
  ...
  messages+=("$(echo_message "Updated Nginx Proxy Manager" false)")
  ...
  execute_command_on_container "rm -rf nginx-proxy-manager-$RELEASE"
